...
GROUP_NAME_ROLE = Qt.UserRole + 1    # extra data role on group-header items
...
class ThemeTree(QTreeWidget):
    ...
    def dropEvent(self, event):
        ...
        if (target is not None
                and target.data(0, Qt.UserRole) is not None   # leaf
                and indicator == QAbstractItemView.OnItem):
            event.ignore()
            return

        # Block: dropping a group header inside itself or a descendant
        if dragged.data(0, Qt.UserRole) is None:   # dragged is a group
            ancestor = target
