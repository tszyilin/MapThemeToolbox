...
# ---------------------------------------------------------------------------
def _fix_catg(raw, title="RORB Catchment"):
    """
    Fix known issues in pyromb-generated .catg text:
      1. Replace line 1 with a proper catchment title.
      2. Change mode indicator from 0 → 1 after C END RORB_GE.
      3. Reformat routing codes: "1,1,kr,-99" → "1, kr,  -99".
      4. Insert "Outlet" name after a bare "7" print code.
    """
    ...
    while i < len(lines):
        ...
        # Fix line 1: catchment title
        if i == 0:
            result.append(f"{title:<69}\n")
            i += 1
            continue

        # Detect end of graphical section
        if 'C END RORB_GE' in stripped:
            past_ge = True
            result.append(line)
            i += 1
            continue

        if past_ge and not past_mode:
            if stripped.startswith('C') or stripped == '':
                result.append(line)
                i += 1
                continue
            # Mode indicator line: should be "1"
            past_mode = True
            result.append("1\n")
            if stripped == '0':
                # Skip the bad "0" line
                i += 1
            # If it's already "1", skip it too
            elif stripped == '1':
                i += 1
            continue

        if past_ge and past_mode:
            # Fix routing code format: "code,1,kr,-99" → "code, kr,  -99"
            m = re.match(r'^([125]),1,([\d.]+),-99', stripped)
            if m:
                code, kr = m.group(1), m.group(2)
                result.append(f"{code}, {kr},  -99\n")
                i += 1
                continue

            # After a bare "7" (print), ensure next line is a node name
            if stripped == '7':
                result.append(line)
                i += 1
                # Check next line
                if i < len(lines):
                    nxt = lines[i].strip()
                    if nxt == '' or nxt == '0':
                        # Insert outlet name before blank/end
                        result.append("Outlet\n")
                        # Don't skip the "0" or blank line
                continue
...
class RorbModelDialog(QDialog):
    ...
    def _on_export_catg(self):
        ...
        try:
            ...
            rm  = {'id': self._reach_row.field("id"),
            ...
            raw = trav.getVector(pyromb.RORB())

            title = self._project_name.text().strip() or "RORB Catchment"
            fixed = _fix_catg(raw, title=title)
