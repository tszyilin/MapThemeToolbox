...
class CreateThemesDialog(QDialog):
    def __init__(self, existing_themes, parent=None):
        ...
        info = QLabel(
            "<b>Note:</b> Each new theme will be created from the "
            "<b>current canvas layer visibility</b>.<br>"
            "Arrange your layers before clicking <b>Create Themes</b>."
        )
    ...
    def _build_csv_tab(self):
        ...
        layout.addWidget(QLabel(
            ...
            "Each row becomes a new theme (created from the current canvas state)."
        ))
    ...
    def _on_create(self):
        ...
        msg = f"Create {len(names)} new theme(s) from current canvas state?"
        if overwrites:
