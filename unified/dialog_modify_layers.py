# -*- coding: utf-8 -*-
"""
Modify Theme Layers — Step 1 theme selector + Step 2 layer toggle.
QGIS 3.22+ and QGIS 4.x.
"""
...
from .compat import (
    Checked, Unchecked, ItemIsUserCheckable,
    ExtendedSelection, DragDrop, MoveAction,
    AlignCenter, CaseInsensitive, MatchContains,
)
...
class ThemeSelectDialog(QDialog):
    def __init__(self, themes, parent=None):
        ...
        layout = QVBoxLayout()
        layout.addWidget(QLabel("<b>Select the themes you want to modify:</b>"))
        sub = QLabel("The next step shows layers visible in ALL selected themes.")
        sub.setWordWrap(True); sub.setStyleSheet("color:#555;")
        ...
        row = QHBoxLayout()
        ba = QPushButton("Select All"); bn = QPushButton("Deselect All")
        ba.clicked.connect(self._select_all); bn.clicked.connect(self._deselect_all)
        row.addWidget(ba); row.addWidget(bn); row.addStretch()
        layout.addLayout(row)

        btns = QHBoxLayout()
        nxt = QPushButton("Next →"); nxt.setStyleSheet("font-weight:bold;")
        cnl = QPushButton("Cancel")
        nxt.clicked.connect(self.accept); cnl.clicked.connect(self.reject)
    ...
    def _select_all(self):
        ...
        for i in range(self.list.count()):
            item = self.list.item(i)
            item.setCheckState(Checked); item.setSelected(True)
        self._syncing = False
    ...
    def _deselect_all(self):
        ...
        self.list.clearSelection()
        for i in range(self.list.count()):
            self.list.item(i).setCheckState(Unchecked)
        self._syncing = False
    ...
    def selected_themes(self):
        return [
            ...
            for i in range(self.list.count())
            if self.list.item(i).checkState() == Checked
        ]
...
class LayerToggleDialog(QDialog):
    def __init__(self, common_layers, selected_themes, all_project_layers, parent=None):
        ...
        instr.setAlignment(AlignCenter)
        ...
        vc = QVBoxLayout()
        vt = QLabel("✅  VISIBLE"); vt.setAlignment(AlignCenter)
        ...
        arr = QLabel("⇄"); arr.setAlignment(AlignCenter)
        arr.setStyleSheet("font-size:22px;color:#888;"); arr.setFixedWidth(28)
        cols.addWidget(arr)

        hc = QVBoxLayout()
        ht = QLabel("🚫  HIDDEN"); ht.setAlignment(AlignCenter)
    ...
    def _completer(self, le):
        ...
        c.setCaseSensitivity(CaseInsensitive)
        c.setFilterMode(MatchContains)
        le.setCompleter(c)
