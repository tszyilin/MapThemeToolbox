# -*- coding: utf-8 -*-
"""
Theme Presenter — dockable panel.  QGIS 3.22+ and QGIS 4.x.
"""
...
from .compat import (
    UserRole, UserRole1,
    InternalMove, MoveAction, OnItem,
    ItemIsEnabled, ItemIsSelectable, ItemIsDragEnabled, ItemIsDropEnabled,
    SingleSelection, ExtendedSelection,
    LeftDock, RightDock, BottomDock,
    DockMovable, DockFloatable, DockClosable,
    Button_Yes, Button_No, Button_Ok, Button_Cancel,
    Accepted,
)

TREE_VAR        = "mtt_tree"
_LEGACY_GRP     = "mtt_theme_groups"
_LEGACY_ORDER   = "mtt_theme_order"
GROUP_NAME_ROLE = UserRole1
...
class ThemeTree(QTreeWidget):
    ...
    def dropEvent(self, event):
        ...
        if (target is not None
                and target.data(0, UserRole) is not None
                and indicator == OnItem):
            event.ignore()
            return

        if dragged.data(0, UserRole) is None:
            ancestor = target
...
class ThemePresenterDock(QDockWidget):
    ...
    def __init__(self, iface, parent=None):
        ...
        self.setAllowedAreas(LeftDock | RightDock | BottomDock)
        self.setFeatures(DockMovable | DockFloatable | DockClosable)
    ...
    def _build_ui(self):
        ...
        self._list.setSelectionMode(SingleSelection)
    ...
    def _build_groups_map(self, node):
        result = {}
        if node["type"] == "group":
            for c in node.get("children", []):
                if c["type"] == "theme":
    ...
    def _is_theme_grouped(self, name):
        return name in self._build_groups_map(self._load_tree())
    ...
    def _rebuild_from_tree(self):
        ...
        tree = {"type": "root", "children": [n for n in children if n]}
    ...
    def _item_to_node(self, item):
        ...
        return {"type": "group", "name": group_name, "children": [n for n in children if n]}

    # ── Core helpers ──────────────────────────────────────────────────────────

    def _tc(self):    return QgsProject.instance().mapThemeCollection()
    def _root(self):  return QgsProject.instance().layerTreeRoot()
    def _model(self): return self.iface.layerTreeView().layerTreeModel()

    def _selected_theme(self):
        item = self._list.currentItem()
        return item.data(0, UserRole) if item else None

    def _selected_group(self):
        item = self._list.currentItem()
        return item.data(0, GROUP_NAME_ROLE) if item else None
    ...
    def _populate_children(self, parent, children, selected, bold, grp_bold):
        for node in children:
            if node["type"] == "group":
                ...
                item.setData(0, UserRole, name)
                item.setFlags(ItemIsEnabled | ItemIsSelectable | ItemIsDragEnabled)
    ...
    def _on_add(self):
        ...
        if name in tc.mapThemes():
            QMessageBox.warning(self, "Already Exists", f"A theme named '{name}' already exists.")
            return
        tc.insert(name, tc.createThemeFromCurrentState(self._root(), self._model()))
    ...
    def _on_rename(self):
        item = self._list.currentItem()
        if item is None:
            QMessageBox.warning(self, "Nothing Selected", "Please click a theme or group header first.")
            return
        theme_name = item.data(0, UserRole)
        group_name = item.data(0, GROUP_NAME_ROLE)
        if theme_name is not None:
            tc = self._tc()
            new, ok = QInputDialog.getText(self, "Rename Theme", "New theme name:", text=theme_name)
            new = new.strip()
            if not ok or not new or new == theme_name:
                return
            if new in tc.mapThemes():
                QMessageBox.warning(self, "Already Exists", f"A theme named '{new}' already exists.")
                return
            ...
            new, ok = QInputDialog.getText(self, "Rename Group", "New group name:", text=group_name)
            new = new.strip()
