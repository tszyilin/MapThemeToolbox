# -*- coding: utf-8 -*-
"""
Sync Table — Excel/CSV ↔ GeoPackage connections.  QGIS 3.22+ and QGIS 4.x.
"""
...
from .compat import (
    AlignLeft, AlignCenter, AlignRight, Horizontal,
    NoEditTriggers, ResizeToContents,
    UserRole, Button_Yes, Button_No,
    FieldType_String, VFW_NoError,
)
...
def _load_excel(path):
    ...
    all_rows = [{headers[i]: (row[i] if i < len(row) else None) for i in range(len(headers))}
                for row in rows[1:]]
    return headers, [r for r in all_rows
...
def _read_file(path):
    ...
    return _load_excel(path) if ext in ('.xlsx', '.xls') else _load_csv(path)
...
def perform_sync(conn, iface, progress_cb=None):
    ...
    if not layer:
        conn.connected = False; REGISTRY.update(conn)
    ...
    fields_to_drop = [
        ...
        if f.lower() not in {s.lower() for s in SYSTEM_FIELDS} and f not in cols
    ]
    ...
    try:
        ...
        if new_fields:
            for col in new_fields:
                if not layer.addAttribute(QgsField(col, FieldType_String)):
    ...
    if not committed:
        errs = [e for e in layer.commitErrors() if not e.upper().startswith("SUCCESS")]
        if errs:
            layer.rollBack()
            return False, "Commit failed: " + "; ".join(errs)

    layer.triggerRepaint()
    try: iface.mapCanvas().refresh()
    except Exception: pass

    from datetime import datetime
    ts  = datetime.now().strftime("%H:%M:%S")
    msg = f"Last sync: {ts}  —  {len(rows)} row(s) inserted"
    conn.last_sync_msg = msg; REGISTRY.update(conn)
    return True, msg
...
class ConnectionPanel(QWidget):
    def __init__(self, conn, iface, parent=None):
        ...
        sp.setContentsMargins(0, 0, 0, 0); sp.setSpacing(6)

        grp1 = QGroupBox("Step 1 — Source File (Excel or CSV)")
        g1   = QHBoxLayout(grp1)
        self._file_label = QLabel("No file selected.")
        self._file_label.setStyleSheet("color:#666;")
        browse_btn = QPushButton("📂  Browse…"); browse_btn.setFixedWidth(100)
        browse_btn.clicked.connect(self._browse_file)
        g1.addWidget(self._file_label, 1); g1.addWidget(browse_btn)
        sp.addWidget(grp1)
        ...
        sw = QVBoxLayout(self._gpkg_select); sw.setContentsMargins(0, 0, 0, 0)
        ...
        self._col_scroll  = QScrollArea(); self._col_scroll.setWidgetResizable(True)
        self._col_scroll.setFixedHeight(70)
        self._col_widget  = QWidget()
        self._col_layout  = QHBoxLayout(self._col_widget)
        self._col_layout.setAlignment(AlignLeft)
        ...
        self._preview.setEditTriggers(NoEditTriggers)
        self._preview.setFixedHeight(110)
        self._preview.horizontalHeader().setSectionResizeMode(ResizeToContents)
        ...
        cp.setContentsMargins(0, 0, 0, 0); cp.setSpacing(6)
        self._banner = QLabel("")
        ...
        reconfig_btn.setStyleSheet("color:#555; font-size:10px;"); reconfig_btn.setFlat(True)
        reconfig_btn.clicked.connect(self._on_reconfigure)
        cp.addWidget(reconfig_btn, alignment=AlignRight)
        ...
        if conn.file_path:
            try:
                ...
                self._refresh_col_checkboxes(); self._refresh_preview()
                self._create_gpkg_btn.setEnabled(True)
            except Exception:
                pass
        self._refresh_phase(); self._refresh_gpkg_ui()
    ...
    def _on_reconfigure(self):
        self._conn.connected = False; REGISTRY.update(self._conn); self._refresh_phase()
    ...
    def _refresh_gpkg_ui(self):
        layers  = self._gpkg_layers(); has_gpkg = bool(layers)
        self._no_gpkg_label.setVisible(not has_gpkg); self._gpkg_select.setVisible(has_gpkg)
        if has_gpkg:
            self._layer_combo.blockSignals(True); self._layer_combo.clear()
            for l in layers: self._layer_combo.addItem(l.name(), l.id())
            if self._conn.layer_id:
                for i in range(self._layer_combo.count()):
                    if self._layer_combo.itemData(i) == self._conn.layer_id:
                        self._layer_combo.setCurrentIndex(i); break
            self._layer_combo.blockSignals(False); self._on_layer_changed()
    ...
    def _on_layer_changed(self):
        ...
        if not layer: self._mode_label.setText(""); return
        ...
        if n == 0:
            ...
            self._mode_label.setText(f"Layer has {n} feature(s) — all will be replaced on sync.")
            self._mode_label.setStyleSheet("color:#155724; font-style:italic; font-size:10px;")
        self._update_connect_btn()

    def _browse_file(self):
        path, _ = QFileDialog.getOpenFileName(self, "Select Excel or CSV", "",
        ...
        self._file_path = path; self._conn.file_path = path; REGISTRY.update(self._conn)
        self._file_label.setText(
            f"{os.path.basename(path)}  —  "
            f"{len(self._file_rows)} row(s), {len(self._file_headers)} col(s)")
        self._file_label.setStyleSheet("color:#155724;")
        self._refresh_col_checkboxes(); self._refresh_preview()
        self._create_gpkg_btn.setEnabled(True); self._update_connect_btn()
    ...
    def _refresh_col_checkboxes(self):
        ...
        for col in self._file_headers:
            ...
            self._col_checkboxes[col] = cb; self._col_layout.addWidget(cb)

    def _refresh_preview(self):
        if not self._file_headers: return
        n = min(5, len(self._file_rows))
        self._preview.setRowCount(n); self._preview.setColumnCount(len(self._file_headers))
    ...
    def _on_connect(self):
        ...
        self._rebuild_banner()
        self._last_sync_label.setText("Connected — click ⚡ Sync Now to push data.")
        self._last_sync_label.setStyleSheet("color:#0d6efd; font-size:10px; padding:2px;")
        self._refresh_phase()
...
class SyncTableDialog(QDialog):
    def __init__(self, iface, parent=None):
        ...
        self.setMinimumSize(760, 520); self.resize(820, 560)

        main_layout = QVBoxLayout(self)
        main_layout.setContentsMargins(8, 8, 8, 8); main_layout.setSpacing(6)

        splitter = QSplitter(Horizontal); splitter.setChildrenCollapsible(False)

        left = QWidget(); left.setMinimumWidth(170); left.setMaximumWidth(230)
        lv   = QVBoxLayout(left); lv.setContentsMargins(0, 0, 4, 0); lv.setSpacing(4)
        lv.addWidget(QLabel("<b>Connections</b>"))
        self._list = QListWidget(); self._list.setSpacing(1)
        ...
        for b in (self._btn_add, self._btn_rename, self._btn_del):
            b.setFixedWidth(32); b.setFixedHeight(26)
        ...
        btn_row.addWidget(self._btn_add); btn_row.addWidget(self._btn_rename)
        btn_row.addWidget(self._btn_del); btn_row.addStretch()
        lv.addLayout(btn_row)
        splitter.addWidget(left)

        self._stack  = QStackedWidget(); self._panels = []
        empty_lbl = QLabel("← Add a connection to get started.")
        empty_lbl.setAlignment(AlignCenter); empty_lbl.setStyleSheet("color:#999;")
        self._empty_widget = empty_lbl; self._stack.addWidget(self._empty_widget)
        splitter.addWidget(self._stack); splitter.setSizes([180, 620])
        ...
        bottom.addWidget(self._sync_all_btn); bottom.addStretch(); bottom.addWidget(close_btn)
    ...
    def closeEvent(self, event):
        REGISTRY.unregister_callback(self._on_registry_changed); super().closeEvent(event)

    def _rebuild_list(self):
        self._list.blockSignals(True); current_row = self._list.currentRow(); self._list.clear()
        ...
        for conn in REGISTRY.connections():
            ...
            item.setData(UserRole, id(conn)); self._list.addItem(item)
            panel = ConnectionPanel(conn, self.iface, parent=self._stack)
            self._stack.addWidget(panel); self._panels.append(panel)
        ...
        if count == 0:
            ...
            self._list.setCurrentRow(row); self._stack.setCurrentIndex(row + 1)
        self._update_sync_all_btn()

    def _item_label(self, conn):
        return f"{'✅  ' if conn.connected else '      '}{conn.name}"

    def _refresh_list_labels(self):
        for i, conn in enumerate(REGISTRY.connections()):
            item = self._list.item(i)
            if item: item.setText(self._item_label(conn))
        self._update_sync_all_btn()
    ...
    def _on_add(self):
        ...
        REGISTRY.add(name); self._rebuild_list()
        self._list.setCurrentRow(self._list.count() - 1)

    def _on_rename(self):
        row = self._list.currentRow(); conns = REGISTRY.connections()
        if row < 0 or row >= len(conns): return
        conn = conns[row]
        name, ok = QInputDialog.getText(self, "Rename Connection", "New name:", text=conn.name)
        name = name.strip()
        if not ok or not name or name == conn.name: return
        conn.name = name; REGISTRY.update(conn)
