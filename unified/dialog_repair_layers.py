# -*- coding: utf-8 -*-
"""
Repair Unavailable Layers.  QGIS 3.22+ and QGIS 4.x.
"""
...
from .compat import (
    HLine, Sunken, ResizeToContents, Stretch,
    NoEditTriggers, SelectRows, ExtendedSelection,
)
...
class RepairLayersDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        hdr.setSectionResizeMode(self.COL_NAME, ResizeToContents)
        hdr.setSectionResizeMode(self.COL_OLD,  Stretch)
        hdr.setSectionResizeMode(self.COL_NEW,  Stretch)
        self._table.setEditTriggers(NoEditTriggers)
        self._table.setSelectionBehavior(SelectRows)
        self._table.setSelectionMode(ExtendedSelection)
        ...
        btn_detect = QPushButton("Auto-detect"); btn_detect.setFixedWidth(100)
        ...
        btn_browse_all = QPushButton("Browse…"); btn_browse_all.setFixedWidth(100)
        btn_browse_all.clicked.connect(lambda: self._browse(self._new_all_edit))
        new_all_row.addWidget(btn_browse_all)
        btn_prev_all = QPushButton("Preview All"); btn_prev_all.setFixedWidth(100)
        ...
        btn_browse_sel = QPushButton("Browse…"); btn_browse_sel.setFixedWidth(100)
        btn_browse_sel.clicked.connect(lambda: self._browse(self._new_sel_edit))
        new_sel_row.addWidget(btn_browse_sel)
        self._btn_prev_sel = QPushButton("Preview Selection"); self._btn_prev_sel.setFixedWidth(130)
        ...
        self._btn_apply.setEnabled(False); self._btn_apply.setToolTip("Re-link all green rows")
        self._btn_apply.clicked.connect(self._apply)
        btn_row.addWidget(self._btn_apply); btn_row.addStretch()
        btn_close = QPushButton("Close"); btn_close.clicked.connect(self.accept)
        btn_row.addWidget(btn_close)
        layout.addLayout(btn_row)

    def _populate(self):
        self._broken = _broken_file_layers(); self._previewed = {}
        ...
        self._auto_detect_all(); self._update_status()

    def _on_selection_changed(self):
        rows = self._selected_rows(); n = len(rows)
        self._sel_label.setText(f"{n} row(s) selected." if n else "No rows selected.")
        self._btn_prev_sel.setEnabled(bool(n))

    def _selected_rows(self):
        return sorted({idx.row() for idx in self._table.selectedIndexes()})

    def _browse(self, edit_widget):
        folder = QFileDialog.getExistingDirectory(self, "Select Folder", edit_widget.text() or "")
        if folder:
            edit_widget.setText(folder)

    def _auto_detect_all(self):
        common = _common_dir([fp for _, fp, _ in self._broken])
        if common:
    ...
    def _preview_all(self):
        old = self._old_edit.text().strip(); new = self._new_all_edit.text().strip()
        if not old or not new:
            QMessageBox.warning(self, "Missing Input", "Please set both Old folder and New folder.")
            return
        for row, (layer, new_src, exists) in enumerate(
                _prefix_replace(self._broken, os.path.normpath(old), os.path.normpath(new))):
            self._set_row_preview(row, new_src, exists, skipped=(new_src is None))

    def _preview_selection(self):
        rows = self._selected_rows()
        if not rows:
            QMessageBox.information(self, "No Selection", "Select rows in the table first.")
            return
        ...
        if not new:
            ...
            return
        old_norm = os.path.normpath(_common_dir([self._broken[r][1] for r in rows]))
        for row, (layer, new_src, exists) in zip(
                rows, _prefix_replace([self._broken[r] for r in rows],
                                      old_norm, os.path.normpath(new))):
            self._set_row_preview(row, new_src, exists, skipped=(new_src is None))
    ...
    def _apply(self):
        ...
        for layer, new_src, exists in self._previewed.values():
            if exists and new_src:
                layer.setDataSource(new_src, layer.name(), layer.dataProvider().name())
                fixed += 1
        if fixed:
