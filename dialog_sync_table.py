...
class ConnectionPanel(QWidget):
    ...
    def __init__(self, conn, iface, parent=None):
        ...
        self._col_layout.setAlignment(Qt.AlignLeft)
        ...
        # ── Init ─────────────────────────────────────────────────────────────
        # If this connection was already connected (restored from settings),
        # verify the layer still exists, then show Phase 2
        if conn.connected:
            layer = QgsProject.instance().mapLayer(conn.layer_id or "")
            if layer:
                ...
                # Layer gone (project not loaded yet etc.)
                conn.connected = False
                REGISTRY.update(conn)
    ...
    def _refresh_gpkg_ui(self):
        ...
        if has_gpkg:
            ...
            # Pre-select the layer this connection was connected to
    ...
    def _create_gpkg(self):
        ...
        try:
            fields_list = [QgsField(h, QVariant.String) for h in self._file_headers]
...
class SyncTableDialog(QDialog):
    ...
    def _rebuild_list(self):
        ...
        for conn in REGISTRY.connections():
            ...
            item.setData(Qt.UserRole, id(conn))
    ...
    def _on_add(self):
        ...
        REGISTRY.add(name)
        self._rebuild_list()
        # Select the new connection (last in list)
    ...
    def _on_sync_all(self):
        ...
        # Refresh labels (last_sync_msg updated)
        self._refresh_list_labels()
